...
<p align="center">
  <strong>7×24 小时帮你干活的全场景个人助理，苏州地铁超级智能体</strong>
</p>
...
**MetroAI** 是苏州地铁超级智能体开发的全场景个人助理 Agent。它 7×24 小时待命，能够帮你完成日常办公中的各类事务 —— 数据分析、制作 PPT、生成视频、撰写文档、搜索信息、收发邮件、定时任务，以及更多。

MetroAI 的核心是 **Cowork 模式**，它能在本地或沙箱环境中执行工具、操作文件、运行命令，一切都在你的监督下自主完成。此外，MetroAI 支持通过微信、企业微信、钉钉、飞书、Telegram、Discord 等 IM 平台远程触发，让你在手机上也能随时指挥 Agent 工作。
...
MetroAI 使用 [OpenClaw](https://github.com/openclaw/openclaw) 作为 Agent 引擎。
...
该运行时以解释器为主，不预装 MetroAI 技能所需的 Python 三方包；相关依赖可在运行时按需安装。
...
MetroAI 采用 Electron 严格进程隔离架构，所有跨进程通信通过 IPC 完成。
...
Cowork 是 MetroAI 的核心功能 —— 以 OpenClaw 为主引擎的 AI 工作会话系统。它面向办公场景设计，能够自主完成数据分析、文档生成、信息检索等复杂任务。
...
MetroAI 内置 29 种技能，覆盖办公、创作、投研、自动化等多种场景，通过 `SKILLs/skills.config.json` 配置启停和排序：
...
MetroAI 支持创建定时任务，让 Agent 按计划自动执行重复性工作。
...
MetroAI 支持将 Agent 桥接到多种 IM 平台。在手机上通过 IM 发送消息即可远程触发桌面端的 Agent 执行任务，随时随地指挥你的个人助理。
...
| 云信 IM | node-nim V2 SDK | P2P 私聊 |
| 小蜜蜂 | node-nim V2 SDK | 个人数字助理 |
| POPO | OpenClaw 网关 | 企业 IM，支持 WebSocket 与 Webhook 两种接入方式 |

在设置面板中配置对应平台的 Token/密钥即可启用。配置完成后，你可以在手机 IM 中直接对 Agent 下达指令（如「帮我分析这份数据」「做一份本周工作汇报 PPT」），Agent 会在桌面端自动执行并返回结果。

## 持久记忆

MetroAI 的记忆系统基于 OpenClaw，以文件形式持久化存储在工作目录中，让 Agent 跨会话记住你的信息和偏好。
...
MetroAI 将 OpenClaw 依赖锁定到指定的 release 版本，在 `package.json` 中声明：
