...
resolve_node_runtime() {
  ...
  if [ -n "${METROAI_ELECTRON_PATH:-}" ] && [ -x "${METROAI_ELECTRON_PATH}" ]; then
    NODE_CMD="$METROAI_ELECTRON_PATH"
    NODE_ARGS=()
...
ensure_npm_available() {
  ...
  echo "  Please reinstall the web-search skill runtime from MetroAI."
  return 1
}
...
if ! resolve_node_runtime; then
  ...
  echo "  Please install Node.js, or run from MetroAI so scripts can use Electron runtime."
  exit 1
fi
