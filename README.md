# MetroAI — All-in-One Personal Assistant Agent

<p align="center">
  <img src="public/logo.png" alt="MetroAI" width="120">
</p>

<p align="center">
  <strong>A 24/7 personal assistant Agent that gets things done, built by Suzhou Metro Super Intelligence</strong>
</p>
...
**MetroAI** is an all-in-one personal assistant Agent. It works around the clock to handle your everyday tasks — data analysis, making presentations, generating videos, writing documents, searching the web, sending emails, scheduling tasks, and more.
...
git clone https://github.com/scottfromcn/MetroAI.git
cd MetroAI
...
MetroAI can use [OpenClaw](https://github.com/openclaw/openclaw) as its agent engine.
...
The bundled runtime is interpreter-focused and does not preinstall MetroAI skill Python packages; those can be installed at runtime on demand.
...
MetroAI uses Electron's strict process isolation. All cross-process communication goes through IPC.
...
Cowork is the core feature of MetroAI — an AI working session system powered by OpenClaw as the primary agent engine. Designed for productivity scenarios, it can autonomously complete complex tasks like data analysis, document generation, and information retrieval.
...
MetroAI ships with 29 built-in skills covering productivity, creative, investment research, and automation scenarios, configured via `SKILLs/skills.config.json`:
...
MetroAI supports scheduled tasks that let the Agent automatically execute recurring work on a set schedule.
...
MetroAI can bridge the Agent to multiple IM platforms. Send a message from your phone via IM to remotely trigger the desktop Agent — command your personal assistant anytime, anywhere.
...
| NetEase IM | node-nim V2 SDK | P2P messaging |
| NetEase Bee | node-nim V2 SDK | Personal digital assistant |
| NetEase POPO | OpenClaw gateway | Enterprise IM, supports WebSocket and Webhook |
...
MetroAI's memory system is built on OpenClaw and persists information as files in the working directory, so the Agent remembers your preferences and context across sessions.
...
MetroAI enforces security at multiple layers:
...
MetroAI pins its OpenClaw dependency to a specific release version, declared in `package.json`:
...
[![Star History Chart](https://api.star-history.com/svg?repos=scottfromcn/MetroAI&type=date&legend=top-left)](https://www.star-history.com/#scottfromcn/MetroAI&type=date&legend=top-left)

---

Built and maintained by Suzhou Metro Super Intelligence.
