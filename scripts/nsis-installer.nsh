...
!macro customInit
  ...
  FileOpen $9 "$APPDATA\MetroAI\install-timing.log" w
  ...
  DetailPrint "[Installer] Stopping running MetroAI processes"
  ...
  IntOp $5 $6 - $7
  FileOpen $9 "$APPDATA\MetroAI\install-timing.log" a
  !insertmacro GetTimestamp $8
  ...
  nsExec::ExecToLog 'powershell -NoProfile -NonInteractive -Command "\
    $$dirs = @(\
      ...
      (Join-Path $$env:APPDATA \"MetroAI\openclaw\state\openclaw-weixin\accounts\")\
    );\
    foreach ($$d in $$dirs) {\
      if (Test-Path $$d) {\
  ...
  ; Copy non-bundled skills to %APPDATA%\MetroAI\skills-backup\ so they are
  ...
  System::Call 'kernel32::GetTickCount()i .r7'
  ClearErrors
  FileOpen $R0 "$APPDATA\MetroAI\skill-migrate.log" w
  ...
  nsExec::ExecToStack 'powershell -NoProfile -NonInteractive -Command "\
    ...
    $$backup = \"$APPDATA\MetroAI\skills-backup\";\
...
!macro customInstall
  ...
  CreateDirectory "$APPDATA\MetroAI"
  FileOpen $2 "$APPDATA\MetroAI\install-timing.log" a
  !insertmacro GetTimestamp $8
  ...
  nsExec::ExecToLog '"$INSTDIR\${APP_EXECUTABLE_FILENAME}" "$INSTDIR\resources\unpack-cfmind.cjs" "$INSTDIR\resources\win-resources.tar" "$INSTDIR\resources" "$APPDATA\MetroAI\install-timing.log"'
  Pop $0
  System::Call 'kernel32::GetTickCount()i .r6'
  IntOp $5 $6 - $7
  ...
  TarExtractProcessFailed:
    ...
    FileClose $2
    MessageBox MB_OK|MB_ICONEXCLAMATION "Resource extraction failed: could not start extractor process (exit=$0). This may be caused by antivirus software. See %APPDATA%\MetroAI\install-timing.log for details."
    Goto TarExtractOK
  ...
  TarExtractNonZero:
    ...
    FileClose $2
    MessageBox MB_OK|MB_ICONEXCLAMATION "Resource extraction failed (exit code $0). See %APPDATA%\MetroAI\install-timing.log for details."
  TarExtractOK:
  ...
  IfFileExists "$APPDATA\MetroAI\skills-backup\*.*" 0 SkipSkillRestore
