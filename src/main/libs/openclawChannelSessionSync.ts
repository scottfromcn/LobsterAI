 ...
 * to local Cowork sessions so that conversations are visible in the MetroAI UI.
 */
...
export class OpenClawChannelSessionSync {
  ...
  resolveOrCreateSession(sessionKey: string): string | null {
    // 1. Skip MetroAI-originated sessions
    if (isManagedSessionKey(sessionKey)) {
      console.log('[ChannelSessionSync] skipped: MetroAI-originated session');
      return null;
    }
